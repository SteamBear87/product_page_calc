$(document).ready(function(){
    ...
    //총 금액이 0이아니라면 .det_total_price를 보여준다.
    ...
    $(document).on("click", ".item_del", function(){
        ...
        $each_calc_price.splice($del_index, 1);    // 해당하는 인덱스 번호의 데이터를 삭제하고 나머지는 저장
        ...
        $(this).closest("li").remove(); //해당 리스트 삭제
        calc_price();
    ...
    // 조건.. 1번 셀렉트 박스가 선택이 된 후에, 2번 셀렉트 박스가 선택되었을때 change이벤트를 걸어 각 세부 항목인 .my_item을 ul아래의 마지막 자식에 추가.
    ...
    $(".form_gram select").change(function(){
        $("#det_box .opt_box").append($each_box);
        ...
        var $opt_02_val = parseFloat($(this).val());  // this ==  ".form_gram select"
        ...
        console.log($each_total_price); // 옵션가 포함한 가격을 배열데이터로 저장(변동값)

        $each_calc_price.push($present_price);
        console.log($each_calc_price); // 내부에서 +또는 -라는 버튼을 클릭했을때 수량과 함께 계산해야할 데이터(고정값)

        $amount.push(1); //숫자형 데이터의 초기값인 1이 들어간다..
    ...
    // 클릭이벤트가 많아서 버그가 생길수 있어서 .on 으로 강제성을 높여주어 지정..
    
    $(document).on("click", ".minus", function(){    //각 아이템 박스의 (-)를 클릭시
    ...
    $(document).on("click", ".plus", function(){    //각 아이템 박스의 (+)를 클릭시
        var $index = $(this).closest("li").index();
        $amount[$index]++;  //수량증가
